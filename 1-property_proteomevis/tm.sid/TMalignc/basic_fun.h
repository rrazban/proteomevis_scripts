				...
				if(line.compare(0, atom.length(), atom)==0)
				{
						...
						line.compare(12, 4, "  CA")==0 )
					{
						...
						if (line.compare(16, 1, "") != 0)
						{
							...
							int numi8 = atoi(pi8);
							if (nres[numi8+20][nDu1] >= 1){// atom0[i][j]: i,j index begin from one	//numi8 can be negative
								mk = -1;
							}
						}
						...
						if (mk == 1)
						{  
							get_xyz(line, &a[i][0], &a[i][1], &a[i][2], &seq[i], &resno[i]);	//issue with resno? returns the same as numi8
				
							nres[resno[i]+20][nDu1] ++;
							i++;
			...
			{			
				if (line.compare(0, atom.length(), atom) == 0)
				{
					...
					if (line.compare(16, 1, " ") != 0)//if(s(17:17).ne.' '), find alternate atom
					{
						...
						const char* pi8 = i8.c_str();
						int numi8 = atoi(pi8);
						int tempNo = nres[numi8][nDu1];// nres[i][j]: i index begins from one, j index begins from 32 to 122	//here?
